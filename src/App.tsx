...
const buildOrderLink = (item: MenuItem, options: { size: SizeKey; quantity: number; addOns: string[]; paymentMethod: PaymentMethod; utr: string; orderId: string }) => {
  const { size, quantity, addOns, paymentMethod, utr, orderId } = options
  ...
  const message = [
    ...
    `🔢 Quantity: ${quantity}`,
    hasKnownPrice ? `💰 Total: ${formatPrice(actualOrderTotal)}` : '💰 Price: Please confirm on WhatsApp',
    '', '🖼️ Product:', toAbsoluteImageUrl(item.image), '', '📍 Delivery: Mangalvedha', '',
    `Payment Method: ${paymentMethod === 'UPI' ? 'UPI' : 'Cash on Delivery'}`,
    paymentMethod === 'UPI' ? `Original Order Total: ${formatPrice(actualOrderTotal)}` : `Amount Payable: ${formatPrice(actualOrderTotal)}`,
    ...(paymentMethod === 'UPI' ? [`${siteConfig.payment.testMode ? 'Test Amount Paid' : 'Amount Paid'}: ${formatPrice(paymentAmount)}`, 'Payment Status: Awaiting Manual Verification', `UPI: ${siteConfig.payment.upiId}`] : []),
    ...(paymentMethod === 'UPI' && utr ? [`UTR: ${utr}`] : []), '',
...
function Navbar({ isOrderPage, menuOpen, onMenuToggle, onMenuClose }: { isOrderPage: boolean; menuOpen: boolean; onMenuToggle: () => void; onMenuClose: () => void }) {
  useEffect(() => {
    if (!menuOpen) return
    const onKey = (event: KeyboardEvent) => { if (event.key === 'Escape') onMenuClose() }
  ...
  return <>
    <header className="topbar">
      ...
      <nav className={`main-nav ${menuOpen ? 'open' : ''}`} aria-label="Main navigation">
        ...
        <a href={isOrderPage ? '/' : '#gallery'} onClick={closeAfterNavigation}>Gallery</a>
...
function PaymentCheckout({ item, size, quantity, addOns, total, hasKnownTotal, paymentMethod, setPaymentMethod, utr, setUtr, paymentSubmitted, setPaymentSubmitted, orderId }: { item: MenuItem; size: SizeKey; quantity: number; addOns: string[]; total: number; hasKnownTotal: boolean; paymentMethod: PaymentMethod | null; setPaymentMethod: Dispatch<SetStateAction<PaymentMethod | null>>; utr: string; setUtr: Dispatch<SetStateAction<string>>; paymentSubmitted: boolean; setPaymentSubmitted: Dispatch<SetStateAction<boolean>>; orderId: string }) {
  ...
  return <>
    <fieldset className="payment-options"><legend>Payment method</legend>
...
function HomePage() {
  ...
  return <><div className="announcement-bar">❤️ Couple Friendly • Cozy Cafe • Mangalvedha</div><main className="page-shell"><Navbar isOrderPage={false} menuOpen={menuOpen} onMenuToggle={() => setMenuOpen((open) => !open)} onMenuClose={() => setMenuOpen(false)} /><section className="hero home-hero" id="home"><div className="hero-copy"><p className="eyebrow">CAFE SRT • MANGALVEDHA</p><div className="premium-badge">♥ Couple Friendly</div><h1>Good Food.<br />Great Vibes.<br /><em>Better Together.</em></h1><p className="hero-subtitle">Your cozy spot in Mangalvedha for coffee dates, casual hangouts and delicious bites.</p><div className="cta-row"><a href="/order" className="primary-btn">Order Food <span>↗</span></a><a href="#location" className="tertiary-btn">Find Us</a></div><div className="hero-meta"><span>♥ Couple Friendly</span><span>☕ Cozy Vibes</span><span>✦ Fresh Bites</span></div></div><div className="hero-visual"><CafePlaceholder label="Cafe SRT cafe illustration" /><div className="hero-sticker" aria-label="Cafe SRT Mangalvedha stamp"><strong>CAFE SRT</strong><span>MANGALVEDHA</span><small>• SIP · EAT · RELAX •</small></div><div className="small-card"><div className="small-card-badge">Good conversations live here</div><p>A comfortable place for coffee dates, relaxed conversations and casual hangouts.</p></div></div></section><section className="experience-strip" aria-label="Cafe experience"><span>♥ <b>Couple Friendly</b></span><span>☕ <b>Cozy Atmosphere</b></span><span>✦ <b>Delicious Bites</b></span><span>⌖ <b>Mangalvedha</b></span></section><section className="about-section split-section" id="about"><div><p className="eyebrow eyebrow-inline">About Cafe SRT</p><h2>A relaxed corner in Mangalvedha.</h2></div><div><p>Cafe SRT brings fast food, beverages and an easygoing cafe environment together in Shivaji Nagar, Mangalvedha.</p><p>Come by for a casual hangout, a coffee date or a quick bite. Delivery is available across Mangalvedha.</p><a className="text-link" href="#location">Find the cafe →</a></div></section><section className="vibe-section"><div className="section-heading"><p className="eyebrow eyebrow-inline">Good Vibes</p><h2>Made for good conversations.</h2><p className="section-lede">Whether it's a coffee date, catching up with friends or a relaxed evening, Cafe SRT gives you a comfortable place to sit, talk and enjoy your favourites.</p></div><div className="vibe-grid"><article><span>♥</span><h3>Couple Friendly</h3><p>A comfortable setting for coffee dates and shared plates.</p></article><article><span>☕</span><h3>Coffee Dates</h3><p>Slow down with a warm drink and a little time together.</p></article><article><span>♧</span><h3>Friends & Hangouts</h3><p>Meet, talk and keep the plans pleasantly casual.</p></article><article><span>✦</span><h3>Snacks & Meals</h3><p>Fast food favourites and beverages for every craving.</p></article></div></section><section className="experience-section" id="gallery"><div className="section-heading"><p className="eyebrow eyebrow-inline">Your Cafe. Your Time.</p><h2>Come as you are. Stay for the vibe.</h2></div><div className="experience-grid"><article><span>☕</span><h3>Coffee & Conversations</h3><p>A warm pause, a familiar table, a good story.</p></article><article><span>🍕</span><h3>Quick Bites</h3><p>Easy favourites for when hunger has plans.</p></article><article><span>◒</span><h3>Evening Hangouts</h3><p>Meet friends and make an ordinary evening better.</p></article></div></section><section className="preview-section"><div className="section-heading row-between"><div><p className="eyebrow eyebrow-inline">Taste What's Waiting</p><h2>Something for every craving.</h2></div><a className="text-link" href="/order">View full menu →</a></div><div className="category-tiles">{['🍕 Pizza','♟ Burgers','▱ Sandwiches','✦ Fries','☕ Cold Coffee','◒ Mocktails'].map((item) => <a className="category-tile" href="/order" key={item}><span>{item.split(' ')[0]}</span><b>{item.substring(item.indexOf(' ') + 1)}</b><i>↗</i></a>)}</div></section><section className="why-section"><div className="section-heading"><p className="eyebrow eyebrow-inline">Why SRT?</p><h2>The little things matter.</h2></div><div className="why-list"><article><b>01</b><div><h3>Couple Friendly</h3><p>A comfortable cafe for relaxed conversations.</p></div></article><article><b>02</b><div><h3>Easy Ordering</h3><p>Browse the menu and continue directly to WhatsApp.</p></div></article><article><b>03</b><div><h3>Fast-Food Favourites</h3><p>Pizza, burgers, sandwiches, drinks and more.</p></div></article><article><b>04</b><div><h3>Mangalvedha Delivery</h3><p>Good food made easy for your local plans.</p></div></article></div></section><section className="delivery-banner"><div><p className="eyebrow eyebrow-inline">Craving Something Good?</p><h2>Browse Cafe SRT's menu and place your order directly on WhatsApp.</h2></div><div className="cta-row"><a className="primary-btn" href="/order">Browse Menu <span>↗</span></a><a className="tertiary-btn light" href={siteConfig.phoneHref}>Call Cafe</a></div></section><section className="location-section" id="location"><div className="location-copy"><p className="eyebrow eyebrow-inline">Find Cafe SRT</p><h2>Your next good plan is here.</h2><p>{siteConfig.address}</p><ul><li>Hours: Open daily • Closing around {siteConfig.closingTime}</li><li>Phone: <a href={siteConfig.phoneHref}>{siteConfig.phoneDisplay}</a></li></ul><div className="cta-row"><a className="primary-btn" href={siteConfig.googleMapsUrl} target="_blank" rel="noreferrer">Get Directions</a><a className="tertiary-btn" href={siteConfig.phoneHref}>Call</a><a className="tertiary-btn" href={siteConfig.whatsappLink} target="_blank" rel="noreferrer">WhatsApp</a></div></div><div className="location-stamp">CAFE SRT<br /><small>SHIVAJI NAGAR<br />MANGALVEDHA</small></div></section><Footer /></main><MobileBar isOrderPage={false} /></>
}

function OrderPage() {
  const [menuOpen, setMenuOpen] = useState(false); const [selectedCategory, setSelectedCategory] = useState('All'); const [searchQuery, setSearchQuery] = useState(''); const [selectedProduct, setSelectedProduct] = useState<MenuItem | null>(null); const [selectedSize, setSelectedSize] = useState<SizeKey>('medium'); const [selectedAddOns, setSelectedAddOns] = useState<string[]>([]); const [quantity, setQuantity] = useState(1); const [paymentMethod, setPaymentMethod] = useState<PaymentMethod | null>(null); const [utr, setUtr] = useState(''); const [paymentSubmitted, setPaymentSubmitted] = useState(false); const [orderId, setOrderId] = useState('')
  const filteredMenu = useMemo(() => menuItems.filter((item) => { const category = ['Cold Coffee', 'Mocktails', 'Tea'].includes(item.category) ? 'Drinks' : ['Bhel', 'Momos'].includes(item.category) ? 'Other' : item.category; return (selectedCategory === 'All' || category === selectedCategory) && item.name.toLowerCase().includes(searchQuery.trim().toLowerCase()) }), [selectedCategory, searchQuery])
...
export default function App() { const isOrderPage = window.location.pathname === '/order' || window.location.pathname.startsWith('/order/'); useEffect(() => { document.title = isOrderPage ? 'Order Food | Cafe SRT Mangalvedha' : 'Cafe SRT Mangalvedha | Couple-Friendly Cafe & Fast Food' }, [isOrderPage]); return isOrderPage ? <OrderPage /> : <HomePage /> }
