...
function Navbar({ isOrderPage, onMenuToggle, menuOpen }: { isOrderPage: boolean; onMenuToggle: () => void; menuOpen: boolean }) {
  return <header className="topbar">
    <a className="brand" href="/" aria-label="Cafe SRT home"><img src={logo} alt="Cafe SRT logo" /><span>Cafe SRT</span></a>
    <button type="button" className="menu-toggle" onClick={onMenuToggle} aria-label="Toggle navigation menu" aria-expanded={menuOpen}>☰</button>
    <nav className={`main-nav ${menuOpen ? 'open' : ''}`} aria-label="Main navigation">
      <a href={isOrderPage ? '/' : '#home'}>Home</a><a href={isOrderPage ? '/#about' : '#about'}>About</a><a href={isOrderPage ? '/' : '#gallery'}>Gallery</a><a href={isOrderPage ? '/#location' : '#location'}>Location</a><a href={isOrderPage ? '/#contact' : '#contact'}>Contact</a>
    </nav>
    <a className={`nav-cta ${isOrderPage ? 'active' : ''}`} href="/order">🍔 Order Food</a>
  </header>
}
